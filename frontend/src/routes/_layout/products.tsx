...
export function ProductsPage() {
    ...
    >({});
    const [indexingProducts, setIndexingProducts] = useState<
    ...
    // START INDEXING JOB (non-blocking)
    const handleIndexImage = async (productId: string) => {
        ...
        try {
            setIndexingProducts((prev) => ({ ...prev, [productId]: true }));
            await indexingJobMutation.mutateAsync({ productId, imageFile });
            toast.success("Image indexing job started!");
            setSelectedImages((prev) => ({ ...prev, [productId]: undefined }));
            refetch(); // refresh products list asynchronously
        } catch {
            toast.error("Failed to start indexing job");
        } finally {
            setIndexingProducts((prev) => ({ ...prev, [productId]: false }));
        }
    };

    const handleImageSelect = (productId: string, file?: File) => {
        if (file) {
                ...
                <div className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p className="text-[#6B7280] mt-2">
                            Create and manage products, then index images for
                            AI-powered search
                        </p>
                    </div>
                    <Button
                    ...
                    {isLoading ? (
                        ...
                        <div className="col-span-full text-center py-12">
                            ...
                            <p className="text-[#6B7280]">
                                Create your first product to get started with AI
                                image indexing
                            </p>
                        </div>
                    ) : (
                        products.map((product: ProductWithImages) => {
							const selectedFile = selectedImages[product.id!];
                            return (
                                <Card
                                    key={product.id}
                                ...
                                >
                                    ...
                                    <div className="aspect-square bg-gray-100 overflow-hidden">
                                        ...
                                        <div
                                            className={`aspect-square bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center ${product.image_ids && product.image_ids.length > 0 ? "hidden" : ""}`}
                                        >
